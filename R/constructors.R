...
initialise_adults <- function(N,pmale){
  ...
  init.dt$gender       <- lapply(init.dt$gender, function(x) x<- rbinom(1,1,1-pmale))
  init.dt$ID           <- 1:N
  init.dt$age          <- lapply(init.dt$age, function (x) x<-round(truncnorm::rtruncnorm(1,mean=20,sd=2,a=14,b=30)))
  init.dt$motherID     <- -1
  init.dt$fatherID     <- -1
  ...
  init.dt$mateID       <- -1
  init.dt$enzyme       <- lapply(init.dt$enzyme, function(x) x <- runif(1, min=0, max=1))  #CHANGE: want to use initial age distribution to get spread of initial enzymes
...
#' gender: Male is 0, female is 1. We know proportions of female/males released
#'          from data.
#' mateID: Unique ID of their mate. Since no initial mosquitoes will have a mate
#'         yet, it is initialised as -1.
#'         Males will always have mateID as -1 since they can have multiple mates.
...
#' typeDeath: Random mortality/trapped death/death due to old age: which type?
#' whereTrapped: in the event of trapped death, where did they die? -1 otherwise.
#' motherID: Unique ID of mother. -1 since wild release.
#' fatherID: Unique ID of father. -1 since wild release.
#' infStatus: 1 if they carry Wolbachia, 0 if no Wolbachia, -1 for CI.
...
#' @return A data.table of \code{N} adult agents.
initialise_release <- function(noReleased, pmale,lat,long){
  ...
  release.dt <- setNames(data.frame(matrix(ncol = noVariables, nrow = noReleased)), c("ID", "gender", "lat","long","mateID", "enzyme","age","gonoCycle","timeDeath","typeDeath","whereTrapped","motherID","fatherID","infStatus","releaseLoc"))
  ...
  release.dt$age        <- lapply(release.dt$age, function(x) x <- round(runif(1,min=18,max=21),0))
  release.dt$motherID   <- -1
  release.dt$fatherID   <- -1
  release.dt$releaseLoc <- -1 #CHECK this needs to be changed to their actual release location
  ...
  #release.dt$infStatus <- 0 #since these are wild mosquitoes, none are infected (from Carla's notes)
  #infStatus: 1 for wolbachia, 0 for no wolbachia, -1 for CI
  release.dt$whereTrapped <- -1

  release.dt$mateID <- -1

  release.dt$enzyme <- lapply(release.dt$enzyme, function(x) x <- runif(1, min=0, max=1))  #CHANGE: want to use initial age distribution to get spread of initial enzymes
